...
public class AroundCircleView extends AppCompatImageView {
    ...
    private static final int DEFAULT_SHADER_START_COLOR =  Color.parseColor("#611475");
    private static final int DEFAULT_SHADER_END_COLOR = Color.parseColor("#DC5FFC");
    ...
    private int mShadeStartColor = DEFAULT_SHADER_START_COLOR;
    private int mShadeEndColor = DEFAULT_SHADER_END_COLOR;
    ...
    public AroundCircleView(Context context, AttributeSet attrs, int defStyle) {
        ...
        mBorderWidth = a.getDimensionPixelSize(
                R.styleable.AroundCircleView_ar_textSize, DEFAULT_BORDER_WIDTH);
        mBorderColor = a.getColor(R.styleable.AroundCircleView_ar_textColor, DEFAULT_BORDER_COLOR);
        mBorderColor_bg = a.getColor(R.styleable.AroundCircleView_ar_textBgColor, DEFAULT_BORDER_COLOR_BG);
        mShadeStartColor = a.getColor(R.styleable.AroundCircleView_shader_start_color,DEFAULT_SHADER_START_COLOR);
        mShadeEndColor = a.getColor(R.styleable.AroundCircleView_shader_end_color,DEFAULT_SHADER_START_COLOR);
        a.recycle();
        mReady = true;
        if (mSetupPending) {
            setup();
...
//            修改该参数,进度条头部设置为圆形
            ...
            //设置进度条颜色渐变
            Shader  mShader = new SweepGradient(getWidth()/2,getHeight(),mShadeStartColor,mShadeEndColor);
            mBorderPaint.setShader(mShader);
            //设置了周边弧度的宽度 每次重新绘制都要画上边上的弧度:设置起始位置
